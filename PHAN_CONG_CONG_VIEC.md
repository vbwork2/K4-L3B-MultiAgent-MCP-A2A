...
- Đầu ra cuối cùng là `outputs/<case_id>.json` cho từng case, `traces/trace.jsonl` và gói `dist/submission-v5.zip`. Không sửa ý nghĩa của các schema trong `contracts/`.
...
- Hoàn thành toàn nhóm khi: `day09 validate-inputs` xác nhận đủ 100 input; `day09 run` tạo đủ 100 output; `day09 validate` pass; `day09 package --output dist/submission-v5.zip` tạo ZIP đúng cấu trúc; nhóm kiểm tra thủ công một số case và cập nhật `ARCHITECTURE.md` theo thiết kế thực tế.
...
9. Chạy `day09 package --output dist/submission-v5.zip`; kiểm tra ZIP chỉ gồm `manifest.json`, `trace.jsonl`, `outputs/<case_id>.json`. Sau khi cả nhóm duyệt, thực hiện bước upload/chọn bản final trên workspace.
